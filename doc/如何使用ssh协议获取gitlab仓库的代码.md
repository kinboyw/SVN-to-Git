...
`SSH` 全称是 **Secure Shell** (**SSH**)，是一种安全加密的网络协议，不同于用户名/密码这种基础验证，SSH 类似于HTTPS，使用非对称加密来来加密连接，在创建连接时会使用 公钥/私钥 的方式来确保对方可信，只需要将客户端的公钥发送给服务器，一旦服务器将客户端的公钥添加到可信任列表中，以后使用 SSH 协议连接服务器时不再需要手动完成认证操作，计算机会自动完成。
